...
var tracery = function() {
	...
	TraceryNode.prototype.expandSequenceChildren = function(childRule, sequence){
		this.children = [];
		this.finishedText = "";
		...
		if (this.childRule !== undefined) {
			...
			for (var i = 0; i < sections.length; i++) {
				...
				this.children[i].expandSequence(sequence);
	...
	TraceryNode.prototype.expandSequence = function(sequence){
		if (!this.isExpanded) {
			...
			switch (this.type) {
				...
				case -1:
					...
					this.expandSequenceChildren(this.raw, sequence);
					break;
				...
				case 1:
					...
					var tempSymbol = this.grammar.symbols[this.symbol];
					var tempRuleSet = tempSymbol.stack[tempSymbol.stack.length - 1].defaultRules;
					var selectedRule = tempRuleSet[sequence[0] % tempRuleSet.length];
					var value = sequence.splice(0, 1)[0];
					sequence.push(value);
					this.expandSequenceChildren(selectedRule, sequence);
	...
	Grammar.prototype.flattenSequence = function(rule, sequence) {
		var root = this.createRoot(rule);
		root.expandSequence(sequence);
		root.clearEscapeChars();
		return root.finishedText;
	}
